...
impl Handler {
    ...
    fn parse_color(value: u16) -> Option<Color> {
        match value {
    ...
    fn as_standard_color(value: u16) -> Option<Color> {
    ...
    fn extract_color(iter: &mut ParamsIter) -> Option<Color> {
        match iter.next()? {
            [2] => {
    ...
    fn update_style(&self, mut codes: ParamsIter) -> TextStyle {
        ...
        loop {
            ...
            match next {
                ...
                30..=37 => {
                    if let Some(color) = Self::as_standard_color(next - 30) {
                        style = style.fg_color(color);
                    }
                }
                40..=47 => {
                    if let Some(color) = Self::as_standard_color(next - 40) {
                        style = style.bg_color(color);
                    }
                }
                38 => {
                    if let Some(color) = Self::extract_color(&mut codes) {
                        style = style.fg_color(color);
                    }
                }
                48 => {
...
impl Perform for Handler {
    ...
    fn csi_dispatch(&mut self, params: &vte::Params, _intermediates: &[u8], _ignore: bool, _action: char) {
        self.save_pending_text();
